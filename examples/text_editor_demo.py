...
class TextEditor:
    ...
    def switch_theme(self, theme_index: int) -> None:
        ...
        if 0 <= theme_index < len(self.THEMES):
            ...
            self.set_message(
                f"Theme: {self.THEMES[theme_index]}",
    ...
    def draw_menu_bar(self) -> None:
        ...
        menu_items = [
            "^N New", "^O Open", "^S Save", "^Q Quit",
            "F1-F8 Themes", "TAB Switch"
        ]
        menu_text = "  ".join(menu_items)
        
        try:
            self.stdscr.addstr(0, 0, " " * self.width, self.theme.components.button)
            self.stdscr.addstr(0, 2, menu_text[:self.width - 4], self.theme.components.button)
        except curses.error:
            pass
    ...
    def draw_status_bar(self) -> None:
        ...
        center_text = f"Line {self.buffer.cursor_row + 1}, Col {self.buffer.cursor_col + 1}"
        ...
        try:
            ...
            self.stdscr.addstr(status_y, 0, " " * self.width, self.theme.components.button_focused)
            
            # Draw left text
            self.stdscr.addstr(status_y, 0, left_text[:self.width // 3], 
                             self.theme.components.button_focused)
            
            # Draw center text
            center_x = (self.width - len(center_text)) // 2
            if center_x > 0:
                self.stdscr.addstr(status_y, center_x, center_text,
                                 self.theme.components.button_focused)
            
            # Draw right text
            right_x = self.width - len(right_text)
            if right_x > 0:
                self.stdscr.addstr(status_y, right_x, right_text,
                                 self.theme.components.button_focused)
        except curses.error:
            pass
    ...
    def draw_editor(self) -> None:
        ...
        for i in range(editor_height):
            ...
            if row_num < len(self.buffer.lines):
                ...
                try:
                    self.stdscr.addstr(
                        y_pos, text_start_x, display_line,
        ...
        for i in range(editor_height):
            try:
                self.stdscr.addstr(editor_top + i, line_num_width, "|",
                                 self.theme.components.border)
            except curses.error:
                pass
        
        # Position cursor
        if (self.scroll_row <= self.buffer.cursor_row < 
            self.scroll_row + editor_height):
    ...
    def draw_file_browser(self) -> None:
        ...
        self.theme.draw_box(
            ...
            browser_width + 1,
            title="Files",
            color_pair=self.theme.components.border if not self.browser_focused
        ...
        for i in range(browser_height):
            ...
            if file_idx < len(self.browser.files):
                ...
                is_dir = os.path.isdir(
                    os.path.join(self.browser.path, filename)
                )
                if is_dir:
