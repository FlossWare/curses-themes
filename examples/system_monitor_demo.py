...
class SystemMonitor:
    ...
    def __init__(self, stdscr):
        ...
        # Available themes
        self.themes = ['default', 'dark', 'light', 'ti-99-4a', 'trs-80', 
                      'dos', 'dbase-iii', 'dbase-iv']
    ...
    def _draw_progress_bar(self, window, y: int, x: int, width: int, 
                          percentage: float, label: str = ""):
        """
        ...
        window.addstr(y, x + width + 1, pct_text, 
                     curses.color_pair(self.theme.colors.foreground))
    
    def _draw_graph(self, window, y: int, x: int, width: int, height: int,
                   data: List[float], max_value: float = 100.0, title: str = ""):
        """
        ...
        scaled_data = [min(int((val / max_value) * height), height - 1) 
    ...
    def _draw_header(self):
        ...
        self.stdscr.addstr(0, 2, theme_name, 
                          curses.color_pair(self.theme.colors.accent))
        
        # Timestamp
        timestamp = time.strftime("%H:%M:%S")
        self.stdscr.addstr(0, width - len(timestamp) - 2, timestamp,
    ...
    def _draw_cpu_panel(self, y: int, x: int, width: int, height: int):
        ...
        # Draw gauge
        self._draw_progress_bar(self.stdscr, y + 2, x + 2, width - 4, 
                               cpu_usage, "Current:")
        
        # Draw graph
        if height > 10:
            self._draw_graph(self.stdscr, y + 5, x + 2, width - 4, height - 7,
                           self.cpu_history, 100.0, "History:")
    ...
    def _draw_memory_panel(self, y: int, x: int, width: int, height: int):
        ...
        self.stdscr.addstr(y + 4, x + 2, mem_info,
                          curses.color_pair(self.theme.colors.foreground))
        
        # Draw graph
        if height > 10:
            self._draw_graph(self.stdscr, y + 6, x + 2, width - 4, height - 8,
                           self.mem_history, 100.0, "History:")
    ...
    def _draw_process_panel(self, y: int, x: int, width: int, height: int):
        ...
        self.stdscr.addstr(y + 2, x + 2, header,
                          curses.color_pair(self.theme.colors.accent) | curses.A_BOLD)
        
        # Draw separator
        self.stdscr.addstr(y + 3, x + 2, '-' * (width - 4),
                          curses.color_pair(self.theme.colors.foreground))
        ...
        for idx, proc in enumerate(processes):
            ...
            try:
                self.stdscr.addstr(row_y, x + 2, row,
    ...
    def _draw_network_panel(self, y: int, x: int, width: int, height: int):
        ...
        self.stdscr.addstr(y + 2, x + 2, upload_text,
                          curses.color_pair(upload_color))
        self.stdscr.addstr(y + 3, x + 2, download_text,
                          curses.color_pair(download_color))
        
        # Draw total bytes
        total_sent = self.last_net_io.bytes_sent / (1024**3)
        total_recv = self.last_net_io.bytes_recv / (1024**3)
        
        self.stdscr.addstr(y + 5, x + 2, f"Total Sent: {total_sent:.2f} GB",
                          curses.color_pair(self.theme.colors.info))
        self.stdscr.addstr(y + 6, x + 2, f"Total Recv: {total_recv:.2f} GB",
                          curses.color_pair(self.theme.colors.info))
    ...
    def _draw_system_panel(self, y: int, x: int, width: int, height: int):
        ...
        # Draw info
        info_y = y + 2
        self.stdscr.addstr(info_y, x + 2, f"Uptime: {uptime}",
        ...
        self.stdscr.addstr(info_y, x + 2, load_text,
                          curses.color_pair(load_color))
        
        info_y += 1
        self.stdscr.addstr(info_y, x + 2, f"CPU Cores: {cpu_count}",
                          curses.color_pair(self.theme.colors.info))
    ...
    def _draw_controls(self):
        ...
        try:
            self.stdscr.addstr(height - 1, (width - len(controls)) // 2, 
                              controls,
    ...
    def _draw_alert(self):
        ...
        for i in range(alert_height):
            try:
                self.stdscr.addstr(alert_y + i + 1, alert_x + 2, ' ' * alert_width,
                                  curses.color_pair(self.theme.colors.foreground) | curses.A_DIM)
            except curses.error:
                pass
        
        # Draw alert box
        self.theme.draw_box(self.stdscr, alert_y, alert_x, alert_height, 
                           alert_width, title="ALERT")
        
        # Draw alert icon
        icon = "⚠"
        self.stdscr.addstr(alert_y + 2, alert_x + (alert_width // 2) - 1, icon,
        ...
        for idx, line in enumerate(msg_lines):
            ...
            self.stdscr.addstr(alert_y + 3 + idx, line_x, line,
                              curses.color_pair(self.theme.colors.foreground))
    ...
    def _draw_all_panels(self):
        ...
        self._draw_process_panel(2, col_width * 2, width - col_width * 2, 
                                 height - 3)
