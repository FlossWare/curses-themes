    ...
    try:
        ...
        if focused:
            ...
            if cursor_x < x + width - 2:
                window.addch(y + 1, cursor_x, '_', 
                           curses.color_pair(color) | curses.A_BLINK)
    except curses.error:
        pass
...
def show_message_box(stdscr, theme, title, message, dialog_type="info"):
    ...
    if hasattr(theme, 'draw_box_3d'):
        theme.draw_box_3d(dialog_win, 0, 0, dialog_height, dialog_width, 
                         raised=True, title=title)
    else:
    ...
    for i, line in enumerate(lines):
        try:
            dialog_win.addstr(2 + i, 6, line[:dialog_width-8], 
...
def show_question_dialog(stdscr, theme, title, question):
    ...
    while True:
        ...
        # Draw question text
        try:
            dialog_win.addstr(2, 6, question[:dialog_width-8], 
                            curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
        ...
        draw_button(dialog_win, theme, button_y, yes_x, "Yes", 
                   focused=(focused_button == 0))
        draw_button(dialog_win, theme, button_y, no_x, "No", 
                   focused=(focused_button == 1))
...
def show_confirmation_dialog(stdscr, theme, title, message):
    ...
    while True:
        ...
        # Draw message
        try:
            dialog_win.addstr(2, 6, message[:dialog_width-8], 
                            curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
        ...
        draw_button(dialog_win, theme, button_y, no_x, "No", 
                   focused=(focused_button == 1))
        draw_button(dialog_win, theme, button_y, cancel_x, "Cancel", 
...
def show_input_dialog(stdscr, theme, title, prompt, default_value=""):
    ...
    while True:
        ...
        draw_input_field(dialog_win, theme, 4, 3, field_width, 
                        input_value, focused=(focused_item == 0))
        ...
        draw_button(dialog_win, theme, button_y, ok_x, "OK", 
                   focused=(focused_item == 1))
        draw_button(dialog_win, theme, button_y, cancel_x, "Cancel", 
                   focused=(focused_item == 2))
...
def show_form_dialog(stdscr, theme, title, fields):
    ...
    focused_item = 0  # 0 to num_fields-1 = fields, num_fields = OK, num_fields+1 = Cancel
    ...
    while True:
        ...
        for i, (label, _) in enumerate(fields):
            ...
            # Draw label
            try:
                dialog_win.addstr(field_y, 3, label + ":", 
                                curses.color_pair(theme.components.foreground))
            except curses.error:
                pass
            ...
            draw_input_field(dialog_win, theme, field_y + 1, max_label_len + 5, 
                           field_width, field_values[i], 
        ...
        if focused_item < num_fields:  # A field has focus
            ...
            if key == ord('\t'):  # Tab
                ...
                focused_item = num_fields if focused_item == num_fields + 1 else num_fields + 1
            elif key == curses.KEY_RIGHT:
                ...
                if focused_item == num_fields:
                    ...
                    result = {label: value for (label, _), value in zip(fields, field_values)}
    ...
    for progress in range(0, 101, 5):
        ...
        try:
            for i in range(fill_width):
                dialog_win.addch(bar_y + 1, bar_x + 2 + i, '█', 
        ...
        try:
            dialog_win.addstr(bar_y + 1, bar_x + bar_width - len(percent_text) - 2, 
                            percent_text, 
...
def show_file_picker_dialog(stdscr, theme, title):
    ...
    while True:
        ...
        try:
            ...
            dialog_win.addstr(2, 3, dir_text[:dialog_width-6], 
                            curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
        ...
        for i in range(max_visible):
            ...
            try:
                dialog_win.addstr(list_y + i, 3, display_text[:dialog_width-6], attrs)
            except curses.error:
                pass
...
def draw_main_screen(stdscr, theme, last_result=""):
    ...
    try:
        stdscr.addstr(2, (width - len(theme_text)) // 2, theme_text, 
                     curses.color_pair(theme.components.foreground))
    except curses.error:
        pass
    ...
    for i, item in enumerate(menu_items):
        try:
            if item.startswith("  ["):
                ...
                stdscr.addstr(start_y + i, 5, item, 
    ...
    if last_result:
        result_y = height - 3
        try:
            stdscr.addstr(result_y, 5, "Last Result: " + last_result, 
...
def main(stdscr):
    ...
    while True:
        ...
        if key == ord('q') or key == ord('Q'):
            ...
            # Info message box
            show_message_box(stdscr, theme, "Information", 
                           "This is an informational message.\nIt can span multiple lines.", 
                           "info")
            last_result = "Info dialog closed"
        elif key == ord('2'):
            # Warning message box
            show_message_box(stdscr, theme, "Warning", 
                           "This is a warning message.\nPlease be careful!", 
                           "warning")
            last_result = "Warning dialog closed"
        elif key == ord('3'):
            # Error message box
            show_message_box(stdscr, theme, "Error", 
                           "An error has occurred!\nPlease check your input.", 
                           "error")
            last_result = "Error dialog closed"
        elif key == ord('4'):
            # Question dialog
            result = show_question_dialog(stdscr, theme, "Confirm Action", 
                                         "Do you want to proceed?")
            last_result = f"Question result: {'Yes' if result == DialogResult.YES else 'No'}"
        elif key == ord('5'):
            # Input dialog
            result, value = show_input_dialog(stdscr, theme, "Enter Name", 
            ...
            result, values = show_form_dialog(stdscr, theme, "Registration Form", fields)
            if result == DialogResult.OK:
            ...
            # Progress dialog
            show_progress_dialog(stdscr, theme, "Processing", 
            ...
            # Confirmation dialog
            result = show_confirmation_dialog(stdscr, theme, "Confirm", 
