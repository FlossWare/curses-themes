...
class CustomThemeRuntime(Theme):
    ...
    def __init__(self, config: ThemeConfig):
        ...
        super().__init__(
            name=config.name, description=config.description, author=config.author
        )
        self.config = config
...
class ThemeWizard:
    ...
    def _init_pickers(self):
        ...
        self.pickers[WizardStep.BACKGROUND] = ColorPicker(
            "Background Color", self.config.background
        )
        self.pickers[WizardStep.FOREGROUND] = ColorPicker(
            "Foreground Color", self.config.foreground
        )
        self.pickers[WizardStep.PRIMARY] = ColorPicker(
            "Primary Color", self.config.primary
        )
        self.pickers[WizardStep.SUCCESS] = ColorPicker(
            "Success Color", self.config.success
        )
        self.pickers[WizardStep.ERROR] = ColorPicker("Error Color", self.config.error)
        self.pickers[WizardStep.WARNING] = ColorPicker(
            "Warning Color", self.config.warning
        )
        self.pickers[WizardStep.INFO] = ColorPicker("Info Color", self.config.info)
        self.pickers[WizardStep.ACCENT] = ColorPicker(
            "Accent Color", self.config.accent
        )
    ...
    def _load_presets(self) -> Dict[str, Dict[str, Tuple[int, int, int]]]:
        ...
        presets["TI-99/4A Colors"] = {
            "background": (28, 60, 148),
            "foreground": (124, 236, 252),
            "primary": (124, 236, 252),
            "success": (0, 255, 0),
            "error": (255, 0, 0),
            "warning": (255, 255, 0),
    ...
    def draw_metadata_step(self):
        ...
        self.content_win.addstr(
            y, 5, "Theme Metadata", curses.A_BOLD | curses.A_UNDERLINE
        )
        y += 2
        ...
        self.content_win.addstr(
            y, 5, "Press Enter to continue to color selection", curses.A_DIM
        )
    ...
    def draw_color_step(self):
        ...
        y = 2
        self.content_win.addstr(
            y, 5, f"Configure {picker.label}", curses.A_BOLD | curses.A_UNDERLINE
        )
        y += 2
    ...
    def draw_border_chars_step(self):
        ...
        y = 2
        self.content_win.addstr(
            y, 5, "Border Character Customization", curses.A_BOLD | curses.A_UNDERLINE
        )
        y += 2
        ...
        self.content_win.addstr(
            y, 5, "Format: 8 characters for TL T TR L R BL B BR", curses.A_DIM
        )
        y += 2
        ...
        if len(self.border_input) == 8:
            ...
            self.content_win.addstr(
                y, 7, "Invalid border characters (need exactly 8)", curses.A_BOLD
            )
    ...
    def draw_preview_step(self):
        ...
        y = 1
        self.content_win.addstr(
            y, 5, f"Theme Preview: {self.config.name}", curses.A_BOLD
        )
        y += 2
        ...
        for label, color_pair in colors_to_show:
            self.content_win.addstr(
                ...
                7,
                f"{label:8s}: ",
                curses.color_pair(self.preview_theme.colors.foreground),
            )
            self.content_win.addstr(
                y,
                18,
                f"Sample text in {label.lower()} color",
        ...
        self.content_win.addstr(
            ...
            10,
            "[ Click Me ]",
            curses.color_pair(self.preview_theme.components.button),
        )
        ...
        self.content_win.addstr(
            ...
            33,
            "[ Click Me ]",
            curses.color_pair(self.preview_theme.components.button_focused),
        )
        ...
        self.content_win.addstr(
            ...
            9,
            "Enter text here...",
            curses.color_pair(self.preview_theme.components.text_input),
        )
        ...
        self.content_win.addstr(
            ...
            9,
            "  Normal item",
            curses.color_pair(self.preview_theme.components.background),
        )
        self.content_win.addstr(
            y + 2,
            9,
            "> Selected item",
            curses.color_pair(self.preview_theme.components.selection),
        )
        self.content_win.addstr(
            y + 3,
            9,
    ...
    def draw_export_step(self):
        ...
        y = 2
        self.content_win.addstr(
            y, 5, "Export Options", curses.A_BOLD | curses.A_UNDERLINE
        )
        y += 2
    ...
    def save_to_json(self):
        ...
        try:
            ...
            self.stdscr.addstr(
                0, 0, f"Saved to {filepath}                    ", curses.A_REVERSE
            )
            self.stdscr.refresh()
            curses.napms(2000)
        except Exception as e:
            self.stdscr.addstr(
                0, 0, f"Error saving: {e}                    ", curses.A_REVERSE
            )
            self.stdscr.refresh()
            curses.napms(2000)
...
from curses_tui import Theme, ColorPair
...
'''
        ...
        try:
            ...
            # Show success message
            self.stdscr.addstr(
                0, 0, f"Exported to {filepath}                    ", curses.A_REVERSE
            )
            self.stdscr.refresh()
            curses.napms(2000)
        except Exception as e:
            self.stdscr.addstr(
                0, 0, f"Error exporting: {e}                    ", curses.A_REVERSE
            )
            self.stdscr.refresh()
            curses.napms(2000)
