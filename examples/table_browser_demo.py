...
def generate_employees_table(count: int = 100) -> List[Dict[str, Any]]:
    ...
    first_names = ["John", "Jane", "Bob", "Alice", "Charlie", "Diana", "Eve", "Frank", 
                   "Grace", "Henry", "Iris", "Jack", "Karen", "Leo", "Mary", "Nancy"]
    last_names = ["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", "Miller",
                  "Davis", "Rodriguez", "Martinez", "Hernandez", "Lopez", "Wilson"]
    departments = ["Engineering", "Sales", "Marketing", "HR", "Finance", "Operations", "IT"]
    titles = ["Manager", "Senior", "Junior", "Lead", "Specialist", "Analyst", "Director"]
...
def generate_customers_table(count: int = 80) -> List[Dict[str, Any]]:
    ...
    companies = ["Acme Corp", "TechStart Inc", "Global Solutions", "DataFlow LLC",
                 "Innovate Systems", "Future Tech", "Digital Dynamics", "Smart Analytics"]
...
def generate_products_table(count: int = 60) -> List[Dict[str, Any]]:
    ...
    products = ["Monitor", "Keyboard", "Mouse", "Desk", "Chair", "Laptop", "Tablet",
                "Printer", "Scanner", "Headset", "Webcam", "Router"]
...
class TableBrowser:
    ...
    def __init__(self, stdscr):
        ...
        self.tables = {
            'EMPLOYEES': {
                ...
                'columns': ['ID', 'First Name', 'Last Name', 'Department', 'Title', 
                           'Hire Date', 'Salary', 'Active']
            },
            'CUSTOMERS': {
                'data': generate_customers_table(),
                'columns': ['ID', 'Company', 'City', 'State', 'Balance', 'Status', 'Records']
            },
            'PRODUCTS': {
                'data': generate_products_table(),
                'columns': ['SKU', 'Product', 'Category', 'In Stock', 'Unit Price', 'Reorder']
            }
        }
    ...
    def get_visible_data(self) -> List[Dict[str, Any]]:
        ...
        if self.filter_text:
            data = [row for row in data 
                   if any(self.filter_text.lower() in str(val).lower() 
    ...
    def draw_header(self):
        ...
        try:
            self.header_win.addstr(0, 2, title, 
                                 curses.color_pair(self.theme.components.button_focused) | 
                                 curses.A_BOLD)
            self.header_win.addstr(0, self.width - len(theme_info) - 2, theme_info,
        ...
        for i, table_name in enumerate(self.table_names):
            if i == self.current_table_idx:
                attr = curses.color_pair(self.theme.components.selection) | curses.A_BOLD
                tab = f"[{table_name}]"
            else:
        ...
        try:
            self.header_win.addstr(2, 0, "=" * (self.width - 1),
    ...
    def draw_table(self):
        ...
        for i, col in enumerate(columns):
            ...
            if i == self.current_col:
                attr = curses.color_pair(self.theme.components.button_focused) | curses.A_BOLD
            else:
        ...
        # Draw separator
        try:
            self.table_win.addstr(1, 0, "-" * (self.width - 1),
                                curses.color_pair(self.theme.components.border))
        except curses.error:
            pass
    ...
    def draw_status_bar(self):
        ...
        try:
            self.status_win.addstr(0, 2, record_info,
                                 curses.color_pair(self.theme.colors.info))
            self.status_win.addstr(0, 25, page_info,
                                 curses.color_pair(self.theme.colors.info))
            if filter_info:
                self.status_win.addstr(0, 40, filter_info,
                                     curses.color_pair(self.theme.colors.warning))
        except curses.error:
            pass
        ...
        try:
            self.status_win.addstr(1, 2, self.status_message,
                                 curses.color_pair(self.theme.components.foreground))
    ...
    def draw_input_line(self):
        ...
        if self.input_mode == 'filter':
            ...
            try:
                self.input_win.addstr(0, 0, text,
                                    curses.color_pair(self.theme.components.text_input))
            except curses.error:
                pass
    ...
    def draw_detail_view(self):
        ...
        if self.detail_win is None:
            self.detail_win = curses.newwin(detail_height, detail_width, detail_y, detail_x)
        ...
        # Draw border
        try:
            self.theme.draw_box(self.detail_win, 0, 0, detail_height, detail_width,
        ...
        for col in table['columns']:
            ...
            try:
                self.detail_win.addstr(y, 2, col + ":",
                                     curses.color_pair(self.theme.components.button) | 
                                     curses.A_BOLD)
                self.detail_win.addstr(y, 2 + len(col) + 2, value,
                                     curses.color_pair(self.theme.components.foreground))
            except curses.error:
                pass
            y += 1
        ...
        # Instructions
        try:
            self.detail_win.addstr(detail_height - 1, 2, "Press Esc or Enter to close",
                                 curses.color_pair(self.theme.colors.accent))
        except curses.error:
            pass
    ...
    def handle_input(self, key):
        ...
        if key == ord('q'):
            ...
            # Next table
            self.current_table_idx = (self.current_table_idx + 1) % len(self.table_names)
            self.current_row = 0
            self.top_row = 0
            self.current_col = 0
            self.filter_text = ""
        ...
        # Navigation keys
            ...
            self.current_row = min(len(data) - 1, 
                                  self.current_row + self.rows_per_page)
            self.top_row = min(max(0, len(data) - self.rows_per_page),
