    ...
    for i in range(1, height):
        if y + i < max_y and x + width < max_x:
            try:
                ...
                if x + width + 1 < max_x:
                    window.addch(y + i, x + width + 1, ' ', curses.color_pair(color_pair))
            except curses.error:
                pass
...
def draw_raised_button(window, theme, y, x, text, focused=False):
    ...
    try:
        window.addstr(y + 1, x + 2, text, curses.color_pair(color) | (curses.A_BOLD if focused else 0))
    except curses.error:
        pass
...
def draw_sunken_input(window, theme, y, x, width, label="", value=""):
    ...
    # Draw input value
    try:
        window.addstr(y + 1, x + 2, value[:width-4], curses.color_pair(theme.components.text_input))
    except curses.error:
        pass
...
def draw_overlapping_windows(stdscr, theme):
    ...
    if height > 20 and width > 70:
        ...
        try:
            stdscr.addstr(10, 13, "This window is behind", curses.color_pair(theme.components.foreground))
            stdscr.addstr(11, 13, "the front window.", curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
    ...
    if height > 18 and width > 65:
        ...
        try:
            stdscr.addstr(8, 23, "Notice the drop shadow", curses.color_pair(theme.components.foreground))
            stdscr.addstr(9, 23, "effects as windows", curses.color_pair(theme.components.foreground))
            stdscr.addstr(10, 23, "overlap each other.", curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
    ...
    if height > 16 and width > 60:
        ...
        try:
            stdscr.addstr(6, 33, "Multiple overlapping", curses.color_pair(theme.components.foreground))
            stdscr.addstr(7, 33, "windows demonstrate", curses.color_pair(theme.components.foreground))
            stdscr.addstr(8, 33, "the 3D shadow effects", curses.color_pair(theme.components.foreground))
            stdscr.addstr(9, 33, "of Borland-style UIs.", curses.color_pair(theme.components.foreground))
        except curses.error:
            pass
...
def draw_demo_screen(stdscr, theme):
    ...
    panel_y = 3
    try:
        stdscr.addstr(panel_y, 2, "RAISED ELEMENTS:", 
    ...
    if width > 50:
        try:
            stdscr.addstr(panel_y, 30, "SUNKEN ELEMENTS:",
                         curses.color_pair(theme.components.selection) | curses.A_BOLD)
        except curses.error:
            pass
        
        # Draw sunken input fields
        draw_sunken_input(stdscr, theme, panel_y + 2, 30, 25, 
                         label="Name:", value="John Doe")
        draw_sunken_input(stdscr, theme, panel_y + 6, 30, 25,
                         label="Email:", value="john@example.com")
        draw_sunken_input(stdscr, theme, panel_y + 10, 30, 25,
                         label="Password:", value="********")
    
    # Bottom section: Overlapping windows demo
    if height > 25:
        try:
            stdscr.addstr(20, 2, "OVERLAPPING WINDOWS WITH SHADOWS:",
                         curses.color_pair(theme.components.selection) | curses.A_BOLD)
        except curses.error:
            pass
    ...
    for i, instruction in enumerate(instructions):
        try:
            ...
            stdscr.addstr(y, (width - len(instruction)) // 2, instruction,
